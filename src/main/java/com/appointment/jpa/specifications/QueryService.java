...
import com.appointment.jpa.specifications.filter.EnumFilter;
import com.appointment.jpa.specifications.filter.Filter;
import com.appointment.jpa.specifications.filter.RangeFilter;
...
public abstract class QueryService<T> {
     ...
     */
    protected <X> Specification<T> buildSpecification(Filter<X> filter, SingularAttribute<? super T, X>
        field) {
     ...
     * @return a Specification
     */
    protected Specification<T> buildStringSpecification(StringFilter filter, SingularAttribute<? super T,
        String> field) {
        return buildSpecification(filter, root -> root.get(field));
    }
     ...
     *
     * @param filter            the individual attribute filter coming from the frontend.
     * @param metaclassFunction lambda, which based on a Root&lt;T&gt; returns Expression - basicaly picks a column
     * @return a Specification
     */
    ...
    /**
     * Helper function to return a specification for filtering on an enum field, where equality, and null/non-null
     * conditions are supported.
     *
     * @param filter the individual attribute filter coming from the frontend.
     * @param field  the JPA static metamodel representing the field.
     * @param <X>    The type of the enum attribute which is filtered.
     * @return a Specification
     */
    protected <X extends Enum<X>> Specification<T> buildSpecification(EnumFilter<X> filter, SingularAttribute<? super T, X> field) {
        return buildSpecification(filter, root -> root.get(field));
    }
     ...
     * @return a Specification
     */
    protected <X extends Enum<X>> Specification<T> buildSpecification(EnumFilter<X> filter, Function<Root<T>, Expression<X>> metaclassFunction) {
        if (filter.getEquals() != null) {
            return equalsSpecification(metaclassFunction, filter.getEquals());
        } else if (filter.getIn() != null) {
            return valueIn(metaclassFunction, filter.getIn());
     ...
     * @return a Specification
     */
    protected <X extends Comparable<? super X>> Specification<T> buildRangeSpecification(RangeFilter<X> filter,
     ...
     * @return a Specification
     */
    protected <X extends Comparable<? super X>> Specification<T> buildSpecification(RangeFilter<X> filter,
     ...
     * @return a Specification
     */
    protected <U, X> Specification<T> buildReferringEntitySpecification(Filter<X> filter,
                                                                                 SingularAttribute<? super T, U> reference,
                                                                                 SingularAttribute<? super U, X> valueField) {
     ...
     * @return a Specification
     */
    protected <U, X> Specification<T> buildReferringEntityStringSpecification(StringFilter filter,
                                                                                 SingularAttribute<? super T, U> reference,
                                                                                 SingularAttribute<? super U, String> valueField) {
        return buildSpecification(filter, root -> root.get(reference).get(valueField));
    }
     ...
     */
    protected <U, X> Specification<T> buildReferringEntitySpecification(Filter<X> filter,
                                                                                 SetAttribute<T, U> reference,
                                                                                 SingularAttribute<U, X> valueField) {
     ...
     *                         checked.
     * @param <U>          The type of the referenced entity.
     * @param <Y>           The type of the entity which is the last before the U in the chain.
     * @param <X>              The type of the attribute which is filtered.
     * @return a Specification
     */
    protected <U, Y, X> Specification<T> buildReferringEntitySpecification(Filter<X> filter,
                                                                                       Function<Root<T>, SetJoin<Y, U>> functionToEntity,
                                                                                       Function<SetJoin<Y, U>, Expression<X>> entityToColumn) {
        if (filter.getEquals() != null) {
     ...
     * @return a Specification
     */
    protected <U, X extends Comparable<? super X>> Specification<T> buildReferringEntitySpecification(final RangeFilter<X> filter,
                                                                                                               final SetAttribute<T, U> reference,
                                                                                                               final SingularAttribute<U, X> valueField) {
        return buildReferringEntitySpecification(filter, root -> root.join(reference), entity -> entity.get(valueField));
    }
     ...
     * @return a Specification
     */
    protected <U, Y, X extends Comparable<? super X>> Specification<T> buildReferringEntitySpecification(final RangeFilter<X> filter,
                                                                                                                     Function<Root<T>, SetJoin<Y, U>> functionToEntity,
                                                                                                                     Function<SetJoin<Y, U>, Expression<X>> entityToColumn) {
    ...
    /**
     * Generic method, which based on a Root&lt;T&gt; returns an Expression which type is the same as the given 'value' type.
     *
     ...
     */
    protected Specification<T> likeUpperSpecification(Function<Root<T>, Expression<String>> metaclassFunction,
                                                           final String value) {
     ...
     * @return a {@link Specification} object.
     */
    protected Specification<T> doesNotContainSpecification(Function<Root<T>, Expression<String>> metaclassFunction,
                                                           final String value) {
     ...
     * @return a {@link Specification} object.
     */
    protected <X> Specification<T> byFieldSpecified(Function<Root<T>, Expression<X>> metaclassFunction,
     ...
     * @return a {@link Specification} object.
     */
    protected <X> Specification<T> byFieldEmptiness(Function<Root<T>, Expression<Set<X>>> metaclassFunction,
                                                         final boolean specified) {
        return specified ?
     ...
     * @return a {@link Specification} object.
     */
    protected <X> Specification<T> valueNotIn(Function<Root<T>, Expression<X>> metaclassFunction,
                                                   final Collection<X> values) {
        return (root, query, builder) -> {
            CriteriaBuilder.In<X> in = builder.in(metaclassFunction.apply(root));
            for (X value : values) {
     ...
     * @return a {@link Specification} object.
     */
    protected <X extends Comparable<? super X>> Specification<T> greaterThanOrEqualTo(Function<Root<T>, Expression<X>> metaclassFunction,
                                                                                           final X value) {
     ...
     * @return a {@link Specification} object.
     */
    protected <X extends Comparable<? super X>> Specification<T> greaterThan(Function<Root<T>, Expression<X>> metaclassFunction,
                                                                                  final X value) {
     ...
     * @return a {@link Specification} object.
     */
    protected <X extends Comparable<? super X>> Specification<T> lessThanOrEqualTo(Function<Root<T>, Expression<X>> metaclassFunction,
                                                                                        final X value) {
     ...
     * @return a {@link Specification} object.
     */
    protected <X extends Comparable<? super X>> Specification<T> lessThan(Function<Root<T>, Expression<X>> metaclassFunction,
                                                                               final X value) {
     ...
     * @return a {@link Specification} object.
     */
    protected Specification<T> distinct(boolean distinct) {
